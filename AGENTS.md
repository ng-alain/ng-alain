...
> 这一节定义 AI 在 NG-ALAIN 中的默认工作方式。目标不是“给出看起来合理的建议”，而是按 Angular 版本、现有架构和可验证结果做决策。

> 更详细的场景模板、决策表和交付检查项请见 [AI_WORKFLOW.md](AI_WORKFLOW.md)。

### 决策顺序

1. 先确认当前仓库的 Angular 版本与现有实现方式，再给出方案；不要把 Angular 新版本能力默认投射到旧实现上。
2. 优先沿用 NG-ALAIN 既有体系：`@delon/*`、NG-ZORRO、Standalone、Zoneless、LESS、路由懒加载、`_HttpClient`、`StartupService`。
3. 只有当现有体系不适合时，才建议替代方案，并说明替代原因与迁移成本。
4. 任何代码生成或重构都要尽量贴近当前目录结构、命名约定和工程脚手架，而不是引入新的抽象层。

### Angular 能力取舍

- 新建页面、组件、服务、指令、管道、路由时，优先使用 Angular CLI 或仓库现有 schematics 生成一致的骨架。
- 新功能如果是表单场景，先评估是否能直接复用 `@delon/form` 或现有表单体系；只有在明确收益更高且与当前架构兼容时，才考虑 Angular 原生 signal forms。
- 状态管理默认使用 Angular Signals；仅在现有 RxJS 流程已经稳定、且改造收益不明确时保留原方式。
- 路由、守卫、数据预取、懒加载、SSR/渲染策略等方案，优先参考 Angular 官方最佳实践，再结合本仓库的守卫与布局约定落地。
- 需要可访问性、动画或组件交互时，优先采用 Angular 推荐的原生能力与语义化结构，再考虑额外封装。

### 代码生成与修改原则

- 生成代码前，先找最接近的现有实现或测试作为锚点，不要无上下文地新造一套模式。
- 组件、服务、守卫、路由等改动要保持 standalone 风格，避免引入 `NgModule`、`zone.js` 或与仓库约定冲突的依赖。
- 对于已有的共享层，只有在 2 个以上业务场景复用时才上升到 shared，否则保持局部导入。
- 涉及 UI 的改动要考虑 NG-ZORRO 和 NG-ALAIN 现有视觉语言，不要为了“现代化”而破坏一致性。

### 交付验证闭环

- 完成代码生成或修改后，默认先做最小可行验证，再扩大范围。
- 优先验证顺序：受影响文件的类型检查/单测，其次是 `npm run build`，最后才是更广泛的 lint 或全量测试。
- 如果改动涉及 Angular 组件模板、signal 状态或路由行为，验证时要关注变更是否真的触发视图更新、路由跳转或守卫逻辑，而不是只看编译通过。
- 验证失败时，先修复当前改动引入的问题，再继续推进，不要把修复范围无关扩散。

### 默认输出要求

- 回答方案时，先说明采用了哪些现有约束，再说明为什么这样做。
- 需要给出代码时，优先给出可直接落地的文件级修改，而不是只描述思路。
- 如果用户要求“完整方案”，默认包含：目标、约束、实现路径、验证方式、迁移风险。
